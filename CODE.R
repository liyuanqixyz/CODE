...
P01N <- as.data.frame(Read10X("data/P01N/"))
colnames(P01N) <- paste0("P01N.",colnames(P01N))

P01T <- as.data.frame(Read10X("data/P01T/"))
colnames(P01T) <- paste0("P01T.",colnames(P01T))

P02N <- as.data.frame(Read10X("data/P02N/"))
colnames(P02N) <- paste0("P02N.",colnames(P02N))

P02T <- as.data.frame(Read10X("data/P02T/"))
colnames(P02T) <- paste0("P02T.",colnames(P02T))

P03N <- as.data.frame(Read10X("data/P03N/"))
colnames(P03N) <- paste0("P03N.",colnames(P03N))

P03T <- as.data.frame(Read10X("data/P03T/"))
colnames(P03T) <- paste0("P03T.",colnames(P03T))

data.list=list(P01N=P01N,P01T=P01T,
               P02N=P02N,P02T=P02T,
               P03N=P03N,P03T=P03T)

rm(P01N,P02N,P03N,P01T,P02T,P03T)

data.list=lapply(data.list, function(x){
  y <- CreateSeuratObject(x) 
  return(y)
})
...
data.list=lapply(data.list, function(x){
  y <- NormalizeData(x) %>%
    ...
    RunPCA() %>%
    RunUMAP(dims = 1:10)
  sweep.res.list_kidney <- paramSweep_v3(y, PCs = 1:10, sct = FALSE)
  sweep.stats_kidney <- summarizeSweep(sweep.res.list_kidney, GT = FALSE)
  bcmvn_kidney <- find.pK(sweep.stats_kidney)
  
  annotations <- y@meta.data$ClusteringResults
  homotypic.prop <- modelHomotypic(annotations) 
  nExp_poi <- round(0.075*nrow(y@meta.data)) 
  nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
  y <- doubletFinder_v3(y, PCs = 1:20, pN = 0.25, pK = 0.01, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
  return(y)
})

filtered.data.list=lapply(data.list, function(x){
  metadata=x@meta.data
  metadata$Batch <- unlist(lapply(strsplit(colnames(x),".",fixed = T),function(x){
    x <- x[1]
    return(x)
  }))
  metadata=metadata[metadata[,5] == 'Singlet',]
  expr=as.data.frame(x@assays$RNA@counts)
  expr=expr[,colnames(expr) %in% rownames(metadata)]
  y=CreateSeuratObject(counts = expr,min.cells = 3, min.features = 200,meta.data=metadata)
  return(y)
})
save(filtered.data.list,file = 'data/filtered_data.RData')

############
load("../data/filtered_data.RData")
s6 <- filtered.data.list
load("../data/filtered_data_2sample.RData")
filtered.data.list <- c(s6,filtered.data.list)
rm(s6)


filtered.data.list <- lapply(filtered.data.list, function(x){
  x[["percent.mt"]] <- PercentageFeatureSet(x, pattern = "^MT-")
  x[["percent.rb"]] <- PercentageFeatureSet(x, pattern = "^RP[SL]")
  return(x)
})

VlnPlot(filtered.data.list[[1]], features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
VlnPlot(filtered.data.list[[1]], features = c("nFeature_RNA", "nCount_RNA", "percent.mt","percent.rb"), ncol = 4)


integrate <- merge(x=filtered.data.list[[1]],y=c(filtered.data.list[[2]],filtered.data.list[[3]],filtered.data.list[[4]],
                                                 filtered.data.list[[5]],filtered.data.list[[6]],filtered.data.list[[7]],
                                                 filtered.data.list[[8]]))

integrate <- subset(integrate, subset = nFeature_RNA < 5000 &nCount_RNA <30000 & percent.mt < 10)

integrate$Group <- unlist(lapply(strsplit(colnames(integrate),".",fixed = T),function(x){
  x <- x[1]
  return(x)
}))

integrate <- NormalizeData(integrate, normalization.method = "LogNormalize", scale.factor = 10000)

integrate <- FindVariableFeatures(integrate, selection.method = "vst", nfeatures = 3000)


s.genes <- cc.genes$s.genes
g2m.genes <- cc.genes$g2m.genes
integrate <- CellCycleScoring(integrate,s.features = s.genes,g2m.features = g2m.genes)

integrate <- ScaleData(integrate,vars.to.regress = c("percent.mt","S.Score","G2M.Score"))
integrate<- RunPCA(integrate, features = VariableFeatures(object = integrate))

library(harmony)
integrate <- RunHarmony(integrate, "Group",max.iter.harmony = 20)

ElbowPlot(integrate,ndims = 50,reduction = "harmony")

integrate <- RunUMAP(integrate,dims = 1:30,reduction = "harmony") %>%
  FindNeighbors(dims = 1:30,reduction = "harmony") %>%
  FindClusters(resolution = .2)

DimPlot(integrate,label = T)
save(integrate,file = '../data/integrate.Rdata')

markers <- FindAllMarkers(integrate,min.pct = 0.3,only.pos = T)
markers=markers[markers$p_val_adj<0.05,]
top10 = markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.csv(markers,row.names = T,"../data/markers.csv")


sce <- integrated
FeaturePlot(sce,features =c("RGS5","COL1A1","ACAT2")) #HSC13
FeaturePlot(sce,features =c('CD79A', 'MS4A1','MZB1')) #B59
FeaturePlot(sce,features =c('CD3D', 'CD3E', 'NKG7'))#NK/T2/0/11
FeaturePlot(sce,features =c( 'CD68', 'CD1C', 'IDO1','LILRA4'))#Myeloid1,3,10,12
FeaturePlot(sce,features =c( 'APOC3', 'SERPINA1', 'ALB'))##hypatocyte 4,17,14,19,6,7
FeaturePlot(sce,features =c(  'PECAM1',"VWF","FCGR2B"))##edothelial8
FeaturePlot(sce,features =c(  'TPSAB1'))##mast18
FeaturePlot(sce,features =c(  'MKI67'))##cycling 16

celltype=data.frame(ClusterID=0:19,
                    celltype=0:19)   
celltype[celltype$ClusterID %in% c(0,11),2]='T cells' 
celltype[celltype$ClusterID %in% c(2),2]='NK cells' 
celltype[celltype$ClusterID %in% c(19,7,14,6,17,4),2]='Hepatocytes'   
celltype[celltype$ClusterID %in% c(8),2]='Endothelial cells' 
celltype[celltype$ClusterID %in% c(1,3,10,12,15,18),2]='Myeloids' 
celltype[celltype$ClusterID %in% c(13),2]="Fibroblasts" 
celltype[celltype$ClusterID %in% c(5),2]='B cells' 
celltype[celltype$ClusterID %in% c(9),2]='Plasma cells'
celltype[celltype$ClusterID %in% c(16),2]='Cyclings'


for(i in 1:nrow(celltype)){
  sce@meta.data[which(sce@meta.data$RNA_snn_res.0.2 == celltype$ClusterID[i]),'Global_Cluster'] <- celltype$celltype[i]}
table(sce@meta.data$Global_Cluster)
save(sce,file = '../data/integrated.final.Rdata')
